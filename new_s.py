...
for main_cat, sub_cats in data.items():
    ...
    sql_lines.append(
        ...
        f"SELECT LAST_INSERT_ID(), {language_id}, '{main_cat}', 'Description for {main_cat}';"
    )
    ...
    sql_lines.append(
        ...
        f"SELECT LAST_INSERT_ID(), cat_id, {language_id}, 'General', 'General image for {main_cat}' "
    ...
    for sub_cat, attributes in sub_cats.items():
        ...
        sql_lines.append(
            f"INSERT INTO child_cats_meta (child_id, child_parent_id, language_id, child_title, child_desc) "
            f"SELECT LAST_INSERT_ID(), cat_id, {language_id}, '{sub_cat}', 'Description for {sub_cat}' "
            f"FROM categories WHERE cat_url = '{cat_url}';"
        )
        ...
        for attr in attributes:
            ...
            sql_lines.append(
                ...
                f"SELECT child_id, cat_id, '{cat_attr}', {language_id} "
                f"FROM categories_children "
                f"JOIN categories ON categories.cat_id = categories_children.child_parent_id "
            ...
            sql_lines.append(
                ...
                f"SELECT cat_attribute.attr_id, cat_attribute.child_id, '{attr}', '{file_path}', {language_id} "
                f"FROM cat_attribute "
                f"JOIN categories_children ON cat_attribute.child_id = categories_children.child_id "
                f"JOIN categories ON categories.cat_id = categories_children.child_parent_id "
                f"WHERE categories.cat_url = '{cat_url}' AND categories_children.child_url = '{child_url}' AND cat_attribute.cat_attr = '{cat_attr}';"
            )
